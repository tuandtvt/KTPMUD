...
window.onload = function () {
	...
	var tags = ["Sân 5", "Sân 7 CT", "Sân 7 CT", "Sân 11 CT", "Sân 11 NT"];
...
function addProductToTable(user) {
	...
	for (var i = 0; i < user.products.length; i++) {
		...
		var thanhtien = stringToNum(price) * 1;
	...
	s += `
			<tr style="font-weight:bold; text-align:center">
				...
				<td class="thanhtoan" onclick="thanhToan()"> Xác nhận </td>
...
function thanhToan() {
	...
	if(c_user.off) {
        alert('Tài khoản của bạn hiện đang bị khóa nên không thể đặt!');
        addAlertBox('Tài khoản của bạn đã bị khóa bởi Admin.', '#aa0000', '#fff', 10000);
        return;
	}
	
	if (!currentuser.products.length) {
		addAlertBox('Không có sân nào cần xác nhận !!', '#ffb400', '#fff', 2000);
		return;
	}
	if (window.confirm('Thanh toán đơn ?')) {
		var order = {
		...
		capNhatMoiThu();
		addAlertBox('Các sân đã được gửi vào đơn và chờ xử lý.', '#17c671', '#fff', 4000);
	}
}

function xoaHet() {
	if (currentuser.products.length) {
		if (window.confirm('Bạn có chắc chắn muốn xóa hết sân trong đơn !!')) {
			currentuser.products = [];
			capNhatMoiThu();
		}
	}
