...
function openTab(nameTab) {
    ...
    switch(nameTab) {
        ...
        case 'Sân': document.getElementsByClassName('sanpham')[0].style.display = 'block'; break;
...
function themSanPham() {
    ...
    for(var p of list_products) {
        if(p.masp == newSp.masp) {
            alert('Mã sân bị trùng !!');
            return false;
        }

        if(p.name == newSp.name) {
            alert('Tên sân bị trùng !!');
            return false;
        }
    }
    ...
    alert('Thêm sân "' + newSp.name + '" thành công.');
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["San5", "San7NT", "San7CT", "San11NT", "San11CT"];
    for(var c of company) {
        if(sp.company == c)
            s += (`<option value="`+c+`" selected>`+c+`</option>`);
        else s += (`<option value="`+c+`">`+c+`</option>`);z
    }
    ...
    s += `
       ...
       <tr>
        ...
        <tr>
            ...
            <td><input type="text" value="`+sp.detail.denchieusang+`"></td>
        </tr>
        <tr>
                ...
                if(duyetDon) {
                    ...
                    if(u[i].donhang[j].tinhTrang == 'Đang chờ xử lý') {
                        if(window.confirm('Bạn có chắc muốn hủy đơn hàng này. Hành động này sẽ không thể khôi phục lại !'))
                            ...
                            //console.log(document.getElementById(maDonHang).parentNode)
                            //document.getElementById(maDonHang).parentNode.style.display='none';
