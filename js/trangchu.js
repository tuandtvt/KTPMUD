window.onload = function () {
	...
	var company = ["San5.jpg", "San7CT.jpg", "San7NT.jpg", "San11CT.jpg", "San11NT.jpg"];
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('Khu vực Hà Nội', yellow_red, ['star=3', 'sort=rateCount-decrease'], soLuong, div);
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
	...
	s += `	</div>
			<a class="xemTatCa" href="index.html?` + filter.join('&') + `" style="` + borderA + `">
				Xem tất cả ` + spResult.length + ` Sân
			</a>
		</div> <hr>`;
